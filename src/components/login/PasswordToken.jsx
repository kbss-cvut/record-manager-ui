...
class PasswordReset extends React.Component {
  ...
  _passwordValid() {
    if (this.state.newPassword.length >= 4 && this.state.newPassword === this.state.confirmPassword) {
    ...
    return (
      <Card variant="info" className="login-panel">
        <Card.Header className="text-light bg-primary" as="h6">
          {this.i18n("login.reset-password")}
        </Card.Header>
        <Card.Body>
          <Form>
            {!this.state.valid && (
            ...
            {this.state.showAlert && passwordChange.status === ACTION_STATUS.ERROR && (
              <AlertMessage
                type={ALERT_TYPES.DANGER}
                alertPosition={"top"}
                message={this.props.formatMessage("user.password-change-error", {
                  error: this.i18n(passwordChange.error.message),
                })}
              />
            )}
            ...
            <div className="login-buttons">
              <Button
                ...
                onClick={this.onSave}
                disabled={
                  this.props.passwordChange.status === ACTION_STATUS.PENDING ||
                  this.props.passwordChange.status === ACTION_STATUS.SUCCESS
                }
              >
                {this.i18n("login.reset-password")}
                {this.props.passwordChange.status === ACTION_STATUS.PENDING && <LoaderSmall />}
              </Button>
              <Button variant="link" onClick={() => transitionTo(Routes.login)}>
                {this.i18n("login.back-to-login")}
              </Button>
            </div>
          </Form>
        </Card.Body>
