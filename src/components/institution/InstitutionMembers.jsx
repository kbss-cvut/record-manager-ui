...
class InstitutionMembers extends React.Component {
  ...
  render() {
    ...
    if (
      ...
      (!institutionMembers.status || institutionMembers.status === ACTION_STATUS.PENDING)
    ) {
      return <Loader />;
    }
    ...
    return (
      <Card variant="info" className="mt-3">
        <Card.Header className="text-light bg-primary" as="h6">
          {this.i18n("institution.members.panel-title")}
        </Card.Header>
        <DeleteItemDialog
  ...
  _renderRows() {
    ...
    for (let i = 0, len = members.length; i < len; i++) {
      const deletionLoading = !!(
        userDeleted.status === ACTION_STATUS.PENDING && userDeleted.username === members[i].username
      );
        ...
        <tr key={member.username}>
          ...
          <td className="report-row actions">
            <Button
              variant="primary"
              size="sm"
              title={this.i18n("users.open-tooltip")}
            ...
            {isAdmin(currentUser) && (
              <Button
                variant="warning"
                size="sm"
                title={this.i18n("users.delete-tooltip")}
                className="action-button"
                onClick={() => this._onDelete(member)}
              >
                {this.i18n("delete")}
                {deletionLoading && <LoaderSmall />}
              </Button>
            )}
          </td>
        </tr>,
