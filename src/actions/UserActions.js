...
export function updateUser(user, currentUser, sendEmail = true) {
  return function (dispatch) {
    ...
    axiosBackend
      .put(`${API_URL}/rest/users/${user.username}${!sendEmail ? "?email=false" : ""}`, {
        ...user,
      })
      .then(() => {
...
export function deleteUser(user, institution = null) {
  return function (dispatch) {
    ...
    return axiosBackend
      .delete(`${API_URL}/rest/users/${user.username}`, {
        ...user,
      })
      .then(() => {
        if (institution) {
...
export function loadUser(username) {
  ...
  return function (dispatch) {
    ...
    return axiosBackend
      .get(`${API_URL}/rest/users/${username}`)
      .then((response) => {
...
export function loadInstitutionMembers(key) {
  ...
  return function (dispatch) {
    ...
    return axiosBackend
      .get(`${API_URL}/rest/users?institution=${key}`)
      .then((response) => {
...
export function changePassword(username, password, sendEmail = true) {
  return function (dispatch, getState) {
    ...
    axiosBackend
      .put(`${API_URL}/rest/users/${username}/password-change${!sendEmail ? "?email=false" : ""}`, {
        ...password,
      })
      .then(() => {
        ...
        dispatch(
          publishMessage(
            successMessage(sendEmail ? "user.password-change-success-with-email" : "user.password-change-success"),
          ),
        );
      })
      .catch((error) => {
        ...
        dispatch(
          publishMessage(
            errorMessage("user.password-change-error", {
              error: getState().intl.messages[error.response.data.messageId],
            }),
          ),
        );
      });
...
export function sendInvitation(username) {
  return function (dispatch) {
    ...
    axiosBackend
      .put(`${API_URL}/rest/users/send-invitation/${username}`)
      .then(() => {
...
export function deleteInvitationOption(username) {
  return function (dispatch) {
    ...
    axiosBackend
      .post(`${API_URL}/rest/users/send-invitation/delete`, username, { headers: { "Content-Type": "text/plain" } })
      .then(() => {
...
export function impersonate(username) {
  return function (dispatch) {
    ...
    axiosBackend
      .post(`${API_URL}/rest/users/impersonate`, `username=${username}`, {
...
export function oidcImpersonate(username) {
  return function (dispatch) {
    ...
    axiosBackend
      .post(
        `${getEnv("AUTH_SERVER_URL")}/protocol/openid-connect/token`,
        new URLSearchParams({
